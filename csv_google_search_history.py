...
from google_html import get_postscount_and_posts_search_history
...
count,postconcat,checkdate_list,scores,intro_extro,judging_function,lifestyle,perceiving_function,type_indicator_analyzer,category_classifier=get_postscount_and_posts_search_history(username,timestamp1,timestamp2)
...
#os.chdir('data_score/')
result.to_csv('/var/www/html/data_score/google_search_history_'+username + '.csv',index=False)
print('You can now download the data at /var/www/html/data_score/google_search_history_'+ username+'.csv')
