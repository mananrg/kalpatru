...
import personality
import urllib
import get_highest_heading_from_url
datastore=""
...
#print(datastore) 
 ...
 headings={}
 list_of_links_and_headings=[]
 while(i<len(datastore)) :
   try: 
    string_for_api_calls=datastore[i]["data"][0]["post"]
    if datastore[i]["attachments"] :
        if datastore[i]["attachments"][0]["data"][0]["external_context"]["url"] :
           link=datastore[i]["attachments"][0]["data"][0]["external_context"]["url"]
           link_for_api_calls=link
           print("Link found in this timestamp")
           print(link)
           time_stamp=datastore[i]["timestamp"]
           headings[link_for_api_calls]=time_stamp
           #headings['timestamp_'+str(time_stamp)+'link_'+str(link)]=json.loads(get_highest_heading_from_url.get_headings(link_for_api_calls))
 ...
 print("*********************************************************************")
 #for i,(x,y) in enumerate(headings.items()):
 #   print(i,x,y)
 return count_of_posts,str_to_return,checkdate_list,scores,dictp,intro_extro,judging_function,lifestyle,perceiving_function,type_indicator_analyzer,category_classifier,headings
...
#get_postscount_and_posts("your_posts_1.json",timestamp1,timestamp2)
#flesch(scores)
